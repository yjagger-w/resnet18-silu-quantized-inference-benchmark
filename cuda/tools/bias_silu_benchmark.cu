...
void print_usage(std::ostream& stream, const char* program) {
    stream
        ...
        << "  --implementation NAME    scalar, float4, auto, or all\n"
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--warmup") {
            ...
            if (value != "scalar"
                ...
                && value != "all") {
                throw std::invalid_argument(
                    "--implementation must be scalar, float4, auto, or all"
                );
            }
...
cudaError_t launch_implementation(
    ...
    if (implementation == "scalar") {
        return silu_cuda::launch_bias_silu_nchw_scalar(
            input,
            bias,
            output,
            shape.batch_size,
    ...
    if (implementation == "float4") {
        return silu_cuda::launch_bias_silu_nchw_vectorized(
            input,
            bias,
            output,
            shape.batch_size,
    ...
    return silu_cuda::launch_bias_silu_nchw(
        input,
        bias,
        output,
        shape.batch_size,
...
double validate_output(
    ...
    for (std::size_t index = 0; index < actual.size(); ++index) {
        ...
        if (absolute_error > allowed_error) {
            throw std::runtime_error(
                "Bias+SiLU output failed correctness validation"
            );
        }
...
std::vector<BenchmarkResult> benchmark_mode_interleaved(
    ...
    float* const launch_input =
        in_place ? device_working : device_source;
    float* const launch_output =
