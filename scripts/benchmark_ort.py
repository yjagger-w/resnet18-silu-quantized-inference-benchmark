...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark ONNX Runtime inference for ResNet18-SiLU."
    )
    parser.add_argument(
        "--onnx",
        type=Path,
    ...
    parser.add_argument(
        ...
        choices=["FP32", "INT8"],
        required=True,
        help="Precision label used in the result table.",
    )
    parser.add_argument(
        "--batch-sizes",
        type=int,
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("results/ort_benchmark.csv"),
        help="CSV output path.",
    )
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("results/ort_benchmark.json"),
...
def main() -> None:
    ...
    print("=" * 60)
    print(f"ONNX Runtime {args.precision} CPU Benchmark")
    print("=" * 60)
